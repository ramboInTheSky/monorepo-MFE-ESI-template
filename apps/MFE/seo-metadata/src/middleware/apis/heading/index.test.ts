...
import headingsRouter, {getHeadingTemplate} from "."
...
function mockSuccessfulAxiosResponse(seoHeading: SeoHeadingsProps | null) {
    const data = seoHeading || null
    const headers = {
...
describe("Given a seo headings api", () => {
    afterEach(() => {
        jest.clearAllMocks()
    })
    ...
    describe("When api returns no data", () => {
        beforeEach(async done => {
            ...
            await getHeadingTemplate(mockRequest as any, mockResponse)

            done()
        })

        it("should call the seo headings api with the correct params", () => {
            expect(axios).toHaveBeenCalledWith({
                url: "superman/api/seo-metadata/Amido/GB/en/v1/seo-headings/Mock/UrlPath",
                method: "get",
            })
        })
...
describe("given a products fragment router", () => {
    it("should define a 'get' route with the correct parameters", () => {
        ...
        headingsRouter(mockExpressRouter as any)

        expect(mockExpressRouter.get).toHaveBeenCalledWith("/seo/seo-headings*?", expect.any(Function))
    })
})
