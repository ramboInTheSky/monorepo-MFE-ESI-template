...
resource "azurerm_resource_group" "header" {
...
module "app_permissions" {
  depends_on       = [azurerm_resource_group.header]
  source           = "git::https://dev.azure.com/Amido-Ecommerce/Amido.Ecommerce.Infrastructure/_git/Amido.Ecommerce.Infrastructure.Modules//terraform/azure/apppermissions"
  resourcegroup_id = azurerm_resource_group.header.id
  domain_group     = var.ad_domain
  environment      = var.name_environment
}

##################################################
# Backend Content Storage - header
resource "azurerm_storage_account" "cdn_header_content_storage" {
  name                      = local.cdn_storage_name
  resource_group_name       = azurerm_resource_group.header.name
  location                  = var.location
...
resource "azurerm_storage_container" "storage_health" {
  ...
  storage_account_name  = azurerm_storage_account.cdn_header_content_storage.name
  container_access_type = "blob"
}

# Static content container for CDN
resource "azurerm_storage_container" "header_content" {
...
resource "azurerm_storage_blob" "header_content_path_health_check_file" {
  name                   = "${var.header_content_path}/healthz.html"
  storage_account_name   = azurerm_storage_account.cdn_header_content_storage.name
...
resource "azurerm_key_vault" "vault" {
  ...
  location            = azurerm_resource_group.header.location
  resource_group_name = azurerm_resource_group.header.name
...
data "azuread_group" "coreteam_domain" {
  display_name = "AzUsers-${lookup(local.AAD_Env_Name_conversion, var.name_environment)}eCommerce-Infra-Contrib"
}
...
resource "azurerm_key_vault_secret" "storageaccountkey" {
  ...
  value        = azurerm_storage_account.cdn_header_content_storage.primary_access_key
  key_vault_id = azurerm_key_vault.vault.id
...
resource "azurerm_key_vault_secret" "storageaccountname" {
  ...
  value        = azurerm_storage_account.cdn_header_content_storage.name
  key_vault_id = azurerm_key_vault.vault.id
  depends_on   = [azurerm_key_vault_access_policy.spn, azurerm_key_vault.vault]
}