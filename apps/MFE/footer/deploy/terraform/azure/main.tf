...
resource "azurerm_resource_group" "footer" {
...
module "app_permissions" {
  depends_on       = [azurerm_resource_group.footer]
  ...
  resourcegroup_id = azurerm_resource_group.footer.id
  domain_group     = var.ad_domain
  environment      = var.name_environment
}

##################################################
# Backend Content Storage - footer
resource "azurerm_storage_account" "cdn_footer_content_storage" {
  name                      = local.cdn_storage_name
  resource_group_name       = azurerm_resource_group.footer.name
  location                  = var.location
...
resource "azurerm_storage_container" "storage_health" {
  ...
  storage_account_name  = azurerm_storage_account.cdn_footer_content_storage.name
  container_access_type = "blob"
}

# Static content container for CDN
resource "azurerm_storage_container" "footer_content" {
...
resource "azurerm_storage_blob" "footer_content_path_health_check_file" {
  name                   = "${var.footer_content_path}/healthz.html"
  storage_account_name   = azurerm_storage_account.cdn_footer_content_storage.name
...
resource "azurerm_key_vault" "vault" {
  ...
  location            = azurerm_resource_group.footer.location
  resource_group_name = azurerm_resource_group.footer.name
...
resource "azurerm_key_vault_secret" "storageaccountkey" {
  ...
  value        = azurerm_storage_account.cdn_footer_content_storage.primary_access_key
  key_vault_id = azurerm_key_vault.vault.id
