...
resource "azurerm_resource_group" "meganav" {
...
module "app_permissions" {
  depends_on       = [azurerm_resource_group.meganav]
  ...
  resourcegroup_id = azurerm_resource_group.meganav.id
  domain_group     = var.ad_domain
  environment      = var.name_environment
}

##################################################
# Backend Content Storage - meganav
resource "azurerm_storage_account" "cdn_meganav_content_storage" {
  name                      = local.cdn_storage_name
  resource_group_name       = azurerm_resource_group.meganav.name
  location                  = var.location
...
resource "azurerm_storage_container" "storage_health" {
  ...
  storage_account_name  = azurerm_storage_account.cdn_meganav_content_storage.name
  container_access_type = "blob"
}

# Static content container for CDN
resource "azurerm_storage_container" "meganav_content" {
...
resource "azurerm_storage_blob" "meganav_content_path_health_check_file" {
  name                   = "${var.meganav_content_path}/healthz.html"
  storage_account_name   = azurerm_storage_account.cdn_meganav_content_storage.name
...
resource "azurerm_key_vault" "vault" {
  ...
  location            = azurerm_resource_group.meganav.location
  resource_group_name = azurerm_resource_group.meganav.name
...
resource "azurerm_key_vault_secret" "storageaccountkey" {
  ...
  value        = azurerm_storage_account.cdn_meganav_content_storage.primary_access_key
  key_vault_id = azurerm_key_vault.vault.id
