...
jest.mock("../../../App.server", () => {
    return {
        ...
        default: jest.fn(async req => {
            ...
            if (req.dataError) return {isDataError: true}
            if (req.isNotModified) return {isNotModified: true}
...
jest.mock("../../../config/env", () => ({
    REACT_APP_CDN_BASEURL: "test app cdn",
    DEVELOPMENT: true,
}))
...
const mockResponse = {sendStatus: jest.fn(_statusCode => ({send: mockSend}))}
...
describe("Given a renderMiddleware", () => {
    describe("When running as middleware", () => {
        ...
        it("should call getServerSideProps", () => {
    ...
    describe("When no data is returned from the API", () => {
        beforeAll(async done => {
            const mockErrorRequest = {dataError: true}
            await renderMiddleware(mockErrorRequest as any, mockResponse as any, mockNext)
            done()
        })
        it("should call response status 204", () => {
            expect(mockResponse.sendStatus).toBeCalledWith(204)
        })
    })

    describe("When a 304 is returned from the API", () => {
        beforeAll(async done => {
            const mockIsNotModifiedRequest = {isNotModified: true}
            await renderMiddleware(mockIsNotModifiedRequest as any, mockResponse as any, mockNext)
            done()
        })
        it("should call response status 304", () => {
            expect(mockResponse.sendStatus).toBeCalledWith(304)
        })
    })
})
