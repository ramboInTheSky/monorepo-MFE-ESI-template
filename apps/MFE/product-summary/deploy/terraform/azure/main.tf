...
resource "azurerm_resource_group" "prodsum" {
...
module "app_permissions" {
  depends_on       = [azurerm_resource_group.prodsum]
  ...
  resourcegroup_id = azurerm_resource_group.prodsum.id
  domain_group     = var.ad_domain
  environment      = var.name_environment
}

##################################################
# Backend Content Storage - prodsum
resource "azurerm_storage_account" "cdn_prodsum_content_storage" {
  name                      = local.cdn_storage_name
  resource_group_name       = azurerm_resource_group.prodsum.name
  location                  = var.location
...
resource "azurerm_storage_container" "storage_health" {
  ...
  storage_account_name  = azurerm_storage_account.cdn_prodsum_content_storage.name
  container_access_type = "blob"
}

# Static content container for CDN
resource "azurerm_storage_container" "prodsum_content" {
...
resource "azurerm_storage_blob" "prodsum_content_path_health_check_file" {
  name                   = "productsummary/healthz.html"
  storage_account_name   = azurerm_storage_account.cdn_prodsum_content_storage.name
...
resource "azurerm_key_vault" "vault" {
  ...
  location            = azurerm_resource_group.prodsum.location
  resource_group_name = azurerm_resource_group.prodsum.name
...
resource "azurerm_key_vault_secret" "storageaccountkey" {
  ...
  value        = azurerm_storage_account.cdn_prodsum_content_storage.primary_access_key
  key_vault_id = azurerm_key_vault.vault.id
...
resource "azurerm_key_vault_secret" "storageaccountname" {
  ...
  value        = azurerm_storage_account.cdn_prodsum_content_storage.name
  key_vault_id = azurerm_key_vault.vault.id
  depends_on   = [azurerm_key_vault_access_policy.spn, azurerm_key_vault.vault]
}