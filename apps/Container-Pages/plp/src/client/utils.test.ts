...
describe("Client App Utils", () => {
    beforeEach(() => {
        ;(window as any).ssrClientSettings = {
            plp: {_STATE_: mockState, appProps: {name: "clark", surname: "kent"}},
        }
    })
    ...
    describe("getStore", () => {
        it("should retrieve the store from the window object when no argument is passed", () => {
            ...
            expect((window as any).ssrClientSettings.plp).not.toHaveProperty("_STATE_")
        })
    ...
    describe("getProps", () => {
        it("should return the app props", () => {
            ...
            expect((window as any).ssrClientSettings.plp).not.toHaveProperty("appProps")
        })
    })
})
