...
describe("themeMiddleware", () => {
    describe("Working scenario with themes available from cache", () => {
        ...
        const mockResponse = {
            locals: {
                configuration: {
                    "monorepo.plp.frontend.appCacheTTL": {Value: 22},
                    "monorepo.plp.frontend.themeVersion": {Value: "v3.3.3"},
                },
            },
        }
    ...
    describe("default version of themes without configuration object inside locals object", () => {
        ...
        it("should fall back v16.0.0 version of themes", () => {
            // eslint-disable-next-line @typescript-eslint/unbound-method
            expect(mockAxios.get).toHaveBeenCalled()

            // eslint-disable-next-line @typescript-eslint/unbound-method
            expect(mockAxios.get).toHaveBeenCalledWith(
                "https://ecmbrowsefdsxeuw.azurefd.net/static-content/theme/v16.0.0/amido_theme.json",
            )
        })
        it("Should call mockNext", () => {
            expect(mockNext).toHaveBeenCalled()
    ...
    describe("Call BFF logger if realm is not provided", () => {
        ...
        it("should call BFF logger warn", () => {
            ...
            // eslint-disable-next-line @typescript-eslint/unbound-method
            expect(BFFLogger.warn).toHaveBeenCalledWith("Error getting realm, defaulting to amido")
        })
        it("Should call mockNext", () => {
            expect(mockNext).toHaveBeenCalled()
        })
    })
    describe("Unable to call axios get, default to amido", () => {
        const cache = {
            get: jest.fn(() => ({
                test: "yeah",
            })),
        ...
        it("should call BFF logger", () => {
            ...
            // eslint-disable-next-line @typescript-eslint/unbound-method
            expect(BFFLogger.warn).toHaveBeenCalledWith("Error getting realm amido, defaulting to amido")
        })
        it("Should call mockNext", () => {
            expect(mockNext).toHaveBeenCalled()
        })
