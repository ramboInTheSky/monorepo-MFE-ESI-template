...
resource "azurerm_resource_group" "plp" {
...
module "app_permissions" {
  depends_on       = [azurerm_resource_group.plp]
  ...
  resourcegroup_id = azurerm_resource_group.plp.id
  domain_group     = var.ad_domain
  environment      = var.name_environment
}

##################################################
# Backend Content Storage - plp
resource "azurerm_storage_account" "cdn_plp_content_storage" {
  name                      = local.cdn_storage_name
  resource_group_name       = azurerm_resource_group.plp.name
  location                  = var.location
...
resource "azurerm_storage_container" "storage_health" {
  ...
  storage_account_name  = azurerm_storage_account.cdn_plp_content_storage.name
  container_access_type = "blob"
}

# Static content container for CDN
resource "azurerm_storage_container" "plp_content" {
...
resource "azurerm_storage_blob" "plp_content_path_health_check_file" {
  name                   = "${var.plp_content_path}/healthz.html"
  storage_account_name   = azurerm_storage_account.cdn_plp_content_storage.name
...
resource "azurerm_key_vault" "vault" {
  ...
  location            = azurerm_resource_group.plp.location
  resource_group_name = azurerm_resource_group.plp.name
...
resource "azurerm_key_vault_secret" "storageaccountkey" {
  ...
  value        = azurerm_storage_account.cdn_plp_content_storage.primary_access_key
  key_vault_id = azurerm_key_vault.vault.id
...
resource "azurerm_key_vault_secret" "storageaccountname" {
  ...
  value        = azurerm_storage_account.cdn_plp_content_storage.name
  key_vault_id = azurerm_key_vault.vault.id
  depends_on   = [azurerm_key_vault_access_policy.spn, azurerm_key_vault.vault]
}